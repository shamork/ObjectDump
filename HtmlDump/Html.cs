		...
		// <meta name="Generator" content="github.com/hyperlinq/hyperlinq"/>
            var doc= H.Doc(null, writeDocType,
                ...
                H.body(
                    new[]
                        {
                            H.div(d => d.id("floating-buttons"),
                                H.button(b => b.type("button").Custom("onclick", "hideToLevel(1)"), "全部折叠"),
                                H.button(b => b.type("button").Custom("onclick", "hideToLevel(2)"), "展开到1层"),
                                H.button(b => b.type("button").Custom("onclick", "hideToLevel(3)"), "展开到2层"),
                                H.button(b => b.type("button").Custom("onclick", "hideToLevel(4)"), "展开到3层"),
                                H.button(b => b.type("button").Custom("onclick", "hideToLevel(1000)"), "全部展开")
                            )
                        }.Concat(bodyChildren)
                        .Concat([H.script(s => s.language("JavaScript").type("text/javascript"), "hideToLevel(1);")])
                        .ToArray()
                )
            );
            return doc;
